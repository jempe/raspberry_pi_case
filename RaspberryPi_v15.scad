...
inside_l = 87.0;                //85.00 is largest length reported, so a bit room on each side
inside_w = 58.0;                //56.17 is largest width  reported, so a bit room on each side
pcb_thickness = 1.6;
...
PCB_holes_diameter = 4; 
PCB_hole1_pos_x = 18;     // Distance from the Micro USB charger corner to the hole
PCB_hole1_pos_y = 25;

PCB_hole2_pos_x = 13;     // Distance from the leds corner to the hole
PCB_hole2_pos_y = 5;
...
module make_pcb() {
    translate(v=[box_w1,box_l1,pcb_h]) {
		difference()
		{
			...
			translate(v=[PCB_hole1_pos_x, inside_l - PCB_hole1_pos_y, 0])
			{
				cylinder(r = PCB_holes_diameter / 2, h = pcb_thickness);
			}
			translate(v=[inside_w - PCB_hole2_pos_x, PCB_hole2_pos_y, 0])
			{
				cylinder(r = PCB_holes_diameter / 2, h = pcb_thickness);
			}
		}
